...
export default function TeamCard({ member }: { member: TeamMember }) {
  ...
  const title = getMetafieldValue(member.metadata?.job_title)
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl transition-shadow duration-300">
      ...
      <div className="p-6">
        ...
        {title && <p className="text-sm font-medium text-primary mt-1">{title}</p>}
        {bio && <p className="mt-3 text-sm text-slate-600 leading-relaxed line-clamp-4">{bio}</p>}
        <div className="mt-4 flex items-center gap-3">
          {email && (
            <a href={`mailto:${email}`} className="text-slate-400 hover:text-primary transition-colors" aria-label="Email">
          ...
          {linkedin && (
            <a href={linkedin} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-primary transition-colors" aria-label="LinkedIn">
